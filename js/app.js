...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (ui.btnGenerate) {
    ui.btnGenerate.addEventListener('click', () => {
      ...
      if (window.matchMedia('(max-width: 960px)').matches) {
        document.querySelector('.generator-main').scrollIntoView({ block: 'start', behavior: 'instant' });
      }
      ...
      if (ui.btnPlayMaster) {
        ...
        ui.btnPlayMaster.setAttribute('aria-label', 'Detener reproducción de la progresión');
      }
    });
  }
  ...
  if (ui.btnPlayMaster) {
    ui.btnPlayMaster.addEventListener('click', () => {
      ...
      if (audio.isPlaying) {
        audio.stop();
        ui.btnPlayMaster.innerHTML = '▶';
        ui.btnPlayMaster.setAttribute('aria-label', 'Reproducir progresión');
      } else {
  ...
  window.addEventListener('popstate', () => {
    ...
    ui.switchTab(['tab-dashboard', 'tab-generator', 'tab-theory'].includes(tab) ? tab : 'tab-dashboard');
  });
